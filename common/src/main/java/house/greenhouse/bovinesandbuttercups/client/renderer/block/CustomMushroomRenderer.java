...
public class CustomMushroomRenderer implements BlockEntityRenderer<CustomMushroomBlockEntity> {
    ...
    public void render(CustomMushroomBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        BakedModel bakedModel = StateDefinitionBovinesModelSetType.getBlockModel(BovinesModelSetRegistry.get(BovinesAndButtercups.asResource("missing_mushroom")), blockEntity.getBlockState());

        if (blockEntity.getMushroomType() != null) {
            @Nullable BovinesModelSet modelSet = BovinesModelSetRegistry.get(blockEntity.getMushroomType().holder().unwrapKey().get().location());
    ...
    @Override
    public boolean shouldRender(CustomMushroomBlockEntity blockEntity, Vec3 cameraPos) {
        return true;
    }
}
