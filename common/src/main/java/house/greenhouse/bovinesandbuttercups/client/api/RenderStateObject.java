    ...
    public static void registerAll() {
        registerGlobalRenderStateObject(ACTIVE_CONDITIONS, entity -> {
            if (entity instanceof LivingEntity living) {
                var cowType = BovinesAndButtercups.getHelper().getCowTypeAttachment(living).cowType();
                return cowType.value().configuration().layers().stream().flatMap(cowModelLayer -> cowModelLayer.textureModifiers().stream().map(textureModifierFactory -> {
