package house.greenhouse.bovinesandbuttercups.client.util;

import house.greenhouse.bovinesandbuttercups.BovinesAndButtercups;
import net.minecraft.client.model.geom.ModelLayerLocation;

public class BovinesModelLayers {
    public static final ModelLayerLocation MOOBLOOM_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("moobloom"), "main");
    public static final ModelLayerLocation BABY_MOOBLOOM_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("moobloom_baby"), "main");

    public static final ModelLayerLocation WARM_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("warm_cow"), "main");
    public static final ModelLayerLocation BABY_WARM_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("warm_cow_baby"), "main");
    public static final ModelLayerLocation COLD_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("cold_cow"), "main");
    public static final ModelLayerLocation BABY_COLD_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("cold_cow_baby"), "main");
    public static final ModelLayerLocation LUSH_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("lush_cow"), "main");
    public static final ModelLayerLocation BABY_LUSH_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("lush_cow_baby"), "main");
    public static final ModelLayerLocation SCULK_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("sculk_cow"), "main");
    public static final ModelLayerLocation BABY_SCULK_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("sculk_cow_baby"), "main");

    public static final ModelLayerLocation BUFFALO_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("buffalo_cow"), "main");
    public static final ModelLayerLocation BABY_BUFFALO_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("buffalo_cow_baby"), "main");
    public static final ModelLayerLocation HIGHLAND_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("highland_cow"), "main");
    public static final ModelLayerLocation BABY_HIGHLAND_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("highland_cow_baby"), "main");
    public static final ModelLayerLocation OX_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("ox_cow"), "main");
    public static final ModelLayerLocation BABY_OX_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("ox_cow_baby"), "main");
    public static final ModelLayerLocation FLAT_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("flat_cow"), "main");
    public static final ModelLayerLocation BABY_FLAT_COW_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("flat_cow_baby"), "main");

    public static final ModelLayerLocation FLOWER_CROWN_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("generic"), "flower_crown");
    public static final ModelLayerLocation PIGLIN_FLOWER_CROWN_MODEL_LAYER = new ModelLayerLocation(BovinesAndButtercups.asResource("piglin"), "flower_crown");
}
