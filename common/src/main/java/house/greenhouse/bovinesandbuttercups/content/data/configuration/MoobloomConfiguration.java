...
public record MoobloomConfiguration(Settings settings,
    ...
    public static final MapCodec<MoobloomConfiguration> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
            ...
            BovinesRegistries.MODEL_TYPE.byNameCodec().optionalFieldOf("model", BovinesCowModelTypes.TEMPERATE).forGetter(MoobloomConfiguration::model),
            ...
            BovinesCodecs.weightedEntryCodec(CowVariant.CODEC, "type").optionalFieldOf("sculk_conversion_types", SimpleWeightedRandomList.empty()).forGetter(MoobloomConfiguration::sculkConverts),
    ...
    public List<WeightedEntry.Wrapper<Holder<CowVariant<MoobloomConfiguration>>>> filterSculkConverts() {
        return (List)sculkConverts.unwrap().stream().filter(holderWrapper -> {
            ...
            if (!bl)
                BovinesAndButtercups.LOG.error("Attempted to add CowType '{}', which does not have CowTypeType '{}' to thunder conversion list.", holderWrapper.data().unwrapKey().map(key -> key.location()).orElse(null), BovinesRegistries.COW_TYPE.getKey(BovinesCowTypes.MOOBLOOM_TYPE));
            return bl;
        }).toList();
    }
