...
public interface CowTypeConfiguration {
     ...
     * @param thunderConverts    A list of weighted cow variants that this cow will/have a chance to convert into upon being struck by lightning.
    ...
    record Settings(Optional<ResourceLocation> cowTexture,
        ...
        public static final MapCodec<Settings> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
                ...
                BovinesCodecs.weightedEntryCodec(RegistryFixedCodec.create(BovinesRegistryKeys.COW_VARIANT), "type").optionalFieldOf("thunder_conversion_types", SimpleWeightedRandomList.empty()).forGetter(Settings::thunderConverts),
        ...
        public <C extends CowTypeConfiguration, T extends CowType<C>> List<WeightedEntry.Wrapper<Holder<CowVariant<C>>>> filterThunderConverts(T type) {
            return (List)thunderConverts.unwrap().stream().filter(holderWrapper -> {
                ...
                if (!bl)
                    BovinesAndButtercups.LOG.error("Attempted to cow variant '{}', which does not have a type of '{}' to thunder conversion list.", holderWrapper.data().unwrapKey().map(key -> key.location()).orElse(null), BovinesRegistries.COW_TYPE.getKey(type));
                return bl;
            }).toList();
        }
    }
