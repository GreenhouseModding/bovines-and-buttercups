...
public record AddCowTypeSpawnsModifier(Holder<CowType<?>> cowType, Optional<HolderSet<Biome>> excludedBiomes, List<MobSpawnSettings.SpawnerData> spawners) implements BiomeModifier {
    ...
    public void modify(Holder<Biome> biome, Phase phase, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        ...
        if (ServerLifecycleHooks.getCurrentServer().registryAccess().lookupOrThrow(BovinesRegistryKeys.COW_VARIANT).stream().anyMatch(entry -> entry.type() == cowType.value() && entry.configuration().settings() != null && entry.configuration().settings().biomes().unwrap().stream().anyMatch(wrapper -> wrapper.data().contains(biome)) && entry.configuration().settings().biomes().unwrap().stream().anyMatch(wrapper -> wrapper.weight().asInt() > 0)) && phase == Phase.ADD && (this.excludedBiomes.isEmpty() || !this.excludedBiomes.get().contains(biome))) {
